...
class Learner:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.num_params <= 0:
            self.log.error(
                "Number of parameters must be greater than zero:"
                + repr(self.num_params)
            )
            raise ValueError
        ...
        if self.min_boundary.shape != (self.num_params,):
            self.log.error(
                "min_boundary array the wrong shape:" + repr(self.min_boundary.shape)
            )
            raise ValueError
        ...
        if self.max_boundary.shape != (self.num_params,):
            self.log.error(
                "max_boundary array the wrong shape:" + self.min_boundary.shape
            )
            raise ValueError
        self.diff_boundary = self.max_boundary - self.min_boundary
        if not np.all(self.diff_boundary > 0.0):
            self.log.error(
                "All elements of max_boundary are not larger than min_boundary"
            )
            raise ValueError
        ...
        if len(self.param_names) != self.num_params:
            message = (
                "param_names has {n_names} elements but there are "
                "{n_params} parameters."
            ).format(n_names=len(self.param_names), n_params=self.num_params)
    ...
    def check_in_boundary(self, param):
        ...
        testbool = np.all(param >= self.min_boundary) and np.all(
            param <= self.max_boundary
        )
        return testbool
    ...
    def put_params_and_get_cost(self, params, **kwargs):
        ...
        if not self.check_num_params(params):
            self.log.error(
                "Incorrect number of parameters sent to queue.Params" + repr(params)
            )
            raise ValueError
        if not self.check_in_boundary(params):
            self.log.warning(
                "Parameters sent to queue are not within boundaries. Params:"
                + repr(params)
            )
    ...
    def save_archive(self):
        ...
        if self.learner_archive_filename is not None:
            mlu.save_dict_to_file(
                self.archive_dict,
                self.total_archive_filename,
                self.learner_archive_file_type,
            )
    ...
    def _set_trust_region(self, trust_region):
        ...
        if trust_region is None:
            ...
            if isinstance(trust_region, float):
                if trust_region > 0 and trust_region < 1:
                    ...
                    self.log.error(
                        "Trust region, when a float, must be between 0 and 1: "
                        + repr(trust_region)
                    )
                    raise ValueError
            else:
        ...
        if self.has_trust_region:
            if not self.check_num_params(self.trust_region):
                self.log.error(
                    "Shape of the trust_region does not match the number of parameters:"
                    + repr(self.trust_region)
                )
                raise ValueError
            if not np.all(self.trust_region > 0):
                self.log.error(
                    "All trust_region values must be positive:"
                    + repr(self.trust_region)
                )
                raise ValueError
            if not self.check_in_diff_boundary(self.trust_region):
                self.log.error(
                    "The trust_region must be smaller than the range of the boundaries:"
                    + repr(self.trust_region)
                )
                raise ValueError
...
class RandomLearner(Learner, threading.Thread):
    ...
    def __init__(self, trust_region=None, first_params=None, **kwargs):
        ...
        if not np.all(self.diff_boundary > 0.0):
            ...
            raise ValueError
        if (
            np.all(np.isfinite(self.min_boundary))
            & np.all(np.isfinite(self.max_boundary))
        ) == False:
            self.log.error(
                "Minimum and/or maximum boundaries are NaN or inf. Must both be finite for random learner. Min boundary:"
                + repr(self.min_boundary)
                + ". Max bounday:"
                + repr(self.max_boundary)
            )
            raise ValueError
        if first_params is None:
            ...
            if not self.check_num_params(self.first_params):
                self.log.error(
                    "first_params has the wrong number of parameters:"
                    + repr(self.first_params)
                )
                raise ValueError
            if not self.check_in_boundary(self.first_params):
                self.log.error(
                    "first_params is not in the boundary:" + repr(self.first_params)
                )
                raise ValueError
    ...
    def run(self):
        ...
        if self.first_params is None:
            next_params = (
                self.min_boundary + nr.rand(self.num_params) * self.diff_boundary
            )
        else:
        ...
        while not self.end_event.is_set():
            try:
                ...
                if self.has_trust_region:
                    temp_min = np.maximum(
                        self.min_boundary, centre_params - self.trust_region
                    )
                    temp_max = np.minimum(
                        self.max_boundary, centre_params + self.trust_region
                    )
                    next_params = temp_min + nr.rand(self.num_params) * (
                        temp_max - temp_min
                    )
                else:
                    next_params = (
                        self.min_boundary
                        + nr.rand(self.num_params) * self.diff_boundary
                    )
...
class NelderMeadLearner(Learner, threading.Thread):
        ...
        **kwargs
        ...
        if initial_simplex_displacements is None and initial_simplex_scale is None:
            ...
            if initial_simplex_scale > 1 or initial_simplex_scale <= 0:
                self.log.error(
                    "initial_simplex_scale must be bigger than 0 and less than 1"
                )
                raise ValueError
            self.init_simplex_disp = self.diff_boundary * initial_simplex_scale
        elif initial_simplex_displacements is not None:
            self.init_simplex_disp = np.array(
                initial_simplex_displacements, dtype=float
            )
        else:
            self.log.error(
                "initial_simplex_displacements and initial_simplex_scale can not both be provided simultaneous."
            )

        if not self.check_num_params(self.init_simplex_disp):
            self.log.error(
                "There is the wrong number of elements in the initial simplex displacement:"
                + repr(self.init_simplex_disp)
            )
            raise ValueError
        if np.any(self.init_simplex_disp < 0):
            self.log.error(
                "initial simplex displacements generated from configuration must all be positive"
            )
            raise ValueError
        if not self.check_in_diff_boundary(self.init_simplex_disp):
            self.log.error(
                "Initial simplex displacements must be within boundaries. init_simplex_disp:"
                + repr(self.init_simplex_disp)
                + ". diff_boundary:"
                + repr(self.diff_boundary)
            )
            raise ValueError

        if initial_simplex_corner is None:
            diff_roll = (self.diff_boundary - self.init_simplex_disp) * nr.rand(
                self.num_params
            )
        ...
        if not self.check_num_params(self.init_simplex_corner):
            self.log.error(
                "There is the wrong number of elements in the initial simplex corner:"
                + repr(self.init_simplex_corner)
            )
        if not self.check_in_boundary(self.init_simplex_corner):
            self.log.error(
                "Initial simplex corner outside of boundaries:"
                + repr(self.init_simplex_corner)
            )
            raise ValueError
        ...
        if not np.all(np.isfinite(self.init_simplex_corner + self.init_simplex_disp)):
            self.log.error(
                "Initial simplex corner and simplex are not finite numbers. init_simplex_corner:"
                + repr(self.init_simplex_corner)
                + ". init_simplex_disp:"
                + repr(self.init_simplex_disp)
            )
            raise ValueError
        if not self.check_in_boundary(
            self.init_simplex_corner + self.init_simplex_disp
        ):
            self.log.error(
                "Largest boundary of simplex not inside the boundaries:"
                + repr(self.init_simplex_corner + self.init_simplex_disp)
            )
            raise ValueError

        self.simplex_params = np.zeros(
            (self.num_params + 1, self.num_params), dtype=float
        )
        self.simplex_costs = np.zeros((self.num_params + 1,), dtype=float)

        self.archive_dict.update(
            {
                "archive_type": "nelder_mead_learner",
    ...
    def run(self):
        ...
        try:
            self.simplex_costs[0] = self.put_params_and_get_cost(
                self.init_simplex_corner
            )
        except ValueError:
            self.log.error(
                "Outside of boundary on initial condition. THIS SHOULD NOT HAPPEN"
            )
            raise
        except LearnerInterrupt:
        ...
        while not self.end_event.is_set():
            ...
            if self.check_in_boundary(xr):
                try:
                    ...
                    self.log.error(
                        "Outside of boundary on first reduce. THIS SHOULD NOT HAPPEN"
                    )
                    raise
                except LearnerInterrupt:
                    break
                ...
                self.log.debug(
                    "Hit boundary (reflect): " + str(self.num_boundary_hits) + " times."
                )
            ...
            if fxr < self.simplex_costs[0]:
                xe = (
                    1 + self.rho * self.chi
                ) * xbar - self.rho * self.chi * self.simplex_params[-1]
                ...
                if self.check_in_boundary(xe):
                    try:
                        ...
                        self.log.error(
                            "Outside of boundary when it should not be. THIS SHOULD NOT HAPPEN"
                        )
                        raise
                    except LearnerInterrupt:
                        break
                ...
                if fxr < self.simplex_costs[-2]:
                    ...
                    if fxr < self.simplex_costs[-1]:
                        xc = (
                            1 + self.psi * self.rho
                        ) * xbar - self.psi * self.rho * self.simplex_params[-1]
                        try:
                            fxc = self.put_params_and_get_cost(xc)
                        except ValueError:
                            self.log.error(
                                "Outside of boundary on contraction: THIS SHOULD NOT HAPPEN"
                            )
                            raise
                        except LearnerInterrupt:
                            break
                        ...
                        try:
                            ...
                            self.log.error(
                                "Outside of boundary on inside contraction: THIS SHOULD NOT HAPPEN"
                            )
                            raise
                        except LearnerInterrupt:
                            break
                    ...
                    if doshrink:
                        for j in one2np1:
                            self.simplex_params[j] = self.simplex_params[
                                0
                            ] + self.sigma * (
                                self.simplex_params[j] - self.simplex_params[0]
                            )
                            try:
                                self.simplex_costs[j] = self.put_params_and_get_cost(
                                    self.simplex_params[j]
                                )
                            except ValueError:
                                self.log.error(
                                    "Outside of boundary on shrink contraction: THIS SHOULD NOT HAPPEN"
                                )
                                raise
                            except LearnerInterrupt:
                                break
...
class DifferentialEvolutionLearner(Learner, threading.Thread):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if len(mutation_scale) == 2 and (
            np.any(np.array(mutation_scale) <= 2)
            or np.any(np.array(mutation_scale) > 0)
        ):
            self.mutation_scale = mutation_scale
        else:
            self.log.error(
                "Mutation scale must be a tuple with (min,max) between 0 and 2. mutation_scale:"
                + repr(mutation_scale)
            )
            raise ValueError
        ...
        if cross_over_probability <= 1 and cross_over_probability >= 0:
            ...
            self.log.error(
                "Cross over probability must be between 0 and 1. cross_over_probability:"
                + repr(cross_over_probability)
            )
        ...
        if population_size >= 5:
            ...
            self.log.error(
                "Population size must be greater or equal to 5:" + repr(population_size)
            )
        ...
        self.archive_dict.update(
            {
                "archive_type": "differential_evolution",
    ...
    def generate_population(self):
        ...
        for index in range(1, self.num_population_members):
            ...
            if self.has_trust_region:
                temp_min = np.maximum(
                    self.min_boundary,
                    self.population[self.min_index] - self.trust_region,
                )
                temp_max = np.minimum(
                    self.max_boundary,
                    self.population[self.min_index] + self.trust_region,
                )
                curr_params = temp_min + nr.rand(self.num_params) * (
                    temp_max - temp_min
                )
            else:
                curr_params = (
    ...
    def mutate(self, index):
        ...
        if self.has_trust_region:
            temp_min = np.maximum(
                self.min_boundary, self.population[self.min_index] - self.trust_region
            )
            temp_max = np.minimum(
                self.max_boundary, self.population[self.min_index] + self.trust_region
            )
        ...
        projected_params = np.where(
            np.logical_or(
                mutated_params < self.min_boundary, mutated_params > self.max_boundary
            ),
            rand_params,
            mutated_params,
        )
    ...
    def _best1(self, index):
        ...
        return self.population[self.min_index] + self.curr_scale * (
    ...
    def _rand1(self, index):
        ...
        return self.population[r0] + self.curr_scale * (
...
class GaussianProcessLearner(Learner, mp.Process):
        ...
        **kwargs
        ...
        if gp_training_filename is not None:
            ...
            if not mlu.check_file_type_supported(gp_training_file_type):
                self.log.error(
                    "GP training file type not supported" + repr(gp_training_file_type)
                )

            self.training_dict = mlu.get_dict_from_file(
                gp_training_filename, gp_training_file_type
            )
            ...
            self.length_scale = mlu.safe_cast_to_array(
                self.training_dict["length_scale"]
            )
            ...
            self.noise_level_history = mlu.safe_cast_to_list(
                self.training_dict["noise_level_history"]
            )
            ...
            self.bad_run_indexs = mlu.safe_cast_to_list(
                self.training_dict["bad_run_indexs"]
            )
            ...
            try:
                self.predicted_best_parameters = mlu.safe_cast_to_array(
                    self.training_dict["predicted_best_parameters"]
                )
                self.predicted_best_cost = float(
                    self.training_dict["predicted_best_cost"]
                )
                self.predicted_best_uncertainty = float(
                    self.training_dict["predicted_best_uncertainty"]
                )
        ...
        if self.length_scale.size == 1:
            ...
            self.log.error(
                "Correlation lengths not the right size and shape, must be one or the number of parameters:"
                + repr(self.length_scale)
            )
            raise ValueError
        if not np.all(self.length_scale > 0):
            self.log.error(
                "Correlation lengths must all be positive numbers:"
                + repr(self.length_scale)
            )
            raise ValueError
        ...
        if self.noise_level < 0:
            self.log.error(
                "noise_level must be greater or equal to zero:" + repr(self.noise_level)
            )
            raise ValueError
        ...
        elif (self.default_bad_cost is not None) and (
            self.default_bad_uncertainty is not None
        ):
            self.bad_defaults_set = True
        else:
            self.log.error(
                "Both the default cost and uncertainty must be set for a bad run or they must both be set to None."
            )
            raise ValueError
        if self.minimum_uncertainty <= 0:
            self.log.error(
                "Minimum uncertainty must be larger than zero for the learner."
            )
            raise ValueError
        ...
        self.archive_dict.update(
            {
                "archive_type": "gaussian_process_learner",
    ...
    def _check_length_scale_bounds(self):
        ...
        if not np.all(bounds > 0):
            message = "Correlation length bounds must all be positive numbers: " + repr(
                self.length_scale_bounds
            )
            self.log.error(message)
            raise ValueError(message)
        dims_error_message = (
            "Length scale bounds must a single pair "
            "(min, max) or a list of pairs [(min_0, max_0), "
            "..., (min_N, max_N)] with one pair per "
            "parameter: " + repr(bounds)
        )
        range_error_message = (
            "The length scale lower bound must be less than "
    ...
    def _check_noise_level_bounds(self):
        ...
        if not np.all(bounds > 0):
            message = "Noise level bounds must all be positive numbers: " + repr(bounds)
            self.log.error(message)
            raise ValueError(message)
        # Ensure that the dimensions are correct.
        if bounds.shape != (2,):
            message = "Noise level bounds should have exactly two elements: " + repr(
                bounds
            )
            self.log.error(message)
            raise ValueError(message)
        ...
        if bounds[1] < bounds[0]:
            message = (
                "Noise level lower bound must be less than or equal to "
    ...
    def get_params_and_costs(self):
        ...
        if self.costs_in_queue.empty():
            if self.end_event.is_set():
                ...
                self.log.error(
                    "Gaussian process asked for new parameters but no new costs were provided."
                )
                raise ValueError
        ...
        while not self.costs_in_queue.empty():
            ...
            if not self.check_num_params(param):
                self.log.error(
                    "Incorrect number of parameters provided to Gaussian process learner:"
                    + repr(param)
            ...
            if not self.check_in_boundary(param):
                self.log.warning(
                    "Parameters provided to Gaussian process learner not in boundaries:"
                    + repr(param)
                )
            cost = float(cost)
            uncer = float(uncer)
            if uncer < 0:
                self.log.error(
                    "Provided uncertainty must be larger or equal to zero:"
                    + repr(uncer)
                )
        ...
        if self.all_params.size == 0:
            ...
            self.all_params = np.concatenate(
                (self.all_params, np.array(new_params, dtype=float))
            )
            self.all_costs = np.concatenate(
                (self.all_costs, np.array(new_costs, dtype=float))
            )
            self.all_uncers = np.concatenate(
                (self.all_uncers, np.array(new_uncers, dtype=float))
            )
        ...
        if self.all_params.shape != (self.costs_count, self.num_params):
            self.log(
                "Saved GP params are the wrong size. THIS SHOULD NOT HAPPEN:"
                + repr(self.all_params)
            )
        if self.all_costs.shape != (self.costs_count,):
            self.log(
                "Saved GP costs are the wrong size. THIS SHOULD NOT HAPPEN:"
                + repr(self.all_costs)
            )
        if self.all_uncers.shape != (self.costs_count,):
            self.log(
                "Saved GP uncertainties are the wrong size. THIS SHOULD NOT HAPPEN:"
                + repr(self.all_uncers)
            )
    ...
    def update_search_region(self):
        ...
        """
        if self.has_trust_region:
            self.search_min = np.maximum(
                self.best_params - self.trust_region, self.min_boundary
            )
            self.search_max = np.minimum(
                self.best_params + self.trust_region, self.max_boundary
            )
            self.search_diff = self.search_max - self.search_min
            self.search_region = list(zip(self.search_min, self.search_max))
    ...
    def update_search_params(self):
        ...
        for _ in range(self.parameter_searches):
            self.search_params.append(
                self.search_min + nr.uniform(size=self.num_params) * self.search_diff
            )
    ...
    def fit_gaussian_process(self):
        ...
        if (
            self.all_params.size == 0
            or self.all_costs.size == 0
            or self.all_uncers.size == 0
        ):
            self.log.error(
                "Asked to fit GP but no data is in all_costs, all_params or all_uncers."
            )
            raise ValueError
        ...
        if self.cost_has_noise:
            ...
            if self._scale_deprecated_noise_levels:
                ...
                self.noise_level_history = [
                    level * cost_scaling_factor ** 2
                    for level in self.noise_level_history
                ]
            ...
            if np.any(np.isnan(self.noise_level_bounds)):
                self.noise_level_bounds = (
                    np.array([1e-5, 1e5]) * cost_scaling_factor ** 2
                )
            ...
            self.scaled_noise_level_bounds = (
                self.noise_level_bounds / cost_scaling_factor ** 2
            )
    ...
    def update_bias_function(self):
        ...
        """
        self.cost_bias = self.bias_func_cost_factor[
    ...
    def predict_biased_cost(self, params):
        ...
        (pred_cost, pred_uncer) = self.gaussian_process.predict(
            params[np.newaxis, :], return_std=True
        )
        ...
        try:
            while not self.end_event.is_set():
                ...
                for _ in range(self.generation_num):
                    self.log.debug(
                        "Gaussian process learner generating parameter:"
                        + str(self.params_count + 1)
                    )
    ...
    def find_global_minima(self):
        ...
        for _ in range(self.parameter_searches):
            search_params.append(
                self.min_boundary
                + nr.uniform(size=self.num_params) * self.diff_boundary
            )
        ...
        for start_params in search_params:
            ...
            (curr_best_cost, curr_best_uncer) = self.gaussian_process.predict(
                curr_best_params[np.newaxis, :], return_std=True
            )
        ...
        self.predicted_best_cost = self.cost_scaler.inverse_transform(
            self.predicted_best_scaled_cost
        )
        self.predicted_best_uncertainty = (
            self.predicted_best_scaled_uncertainty * self.cost_scaler.scale_
        )
...
class NeuralNetLearner(Learner, mp.Process):
        ...
        **kwargs
        ...
        if nn_training_filename is not None:
            ...
            if not mlu.check_file_type_supported(nn_training_file_type):
                self.log.error(
                    "NN training file type not supported" + repr(nn_training_file_type)
                )
            self.nn_training_file_dir = os.path.dirname(nn_training_filename)

            self.training_dict = mlu.get_dict_from_file(
                nn_training_filename, nn_training_file_type
            )
    ...
    def _fit_neural_net(self, index):
        ...
        """
        self.scaled_costs = self.cost_scaler.transform(self.all_costs[:, np.newaxis])[
            :, 0
        ]
    ...
    def predict_cost_gradient(self, params, net_index=None):
        ...
        return (
            self.neural_net[net_index].predict_cost_gradient(params).astype(np.float64)
        )
    ...
    def get_params_and_costs(self):
        ...
        while True:
            if first_dequeue:
                try:
                    ...
                    (param, cost, uncer, bad) = self.costs_in_queue.get(
                        block=True, timeout=1
                    )
                    first_dequeue = False
                except mlu.empty_exception:
                    self.log.error(
                        "Neural network asked for new parameters but no new costs were provided after 1s."
                    )
                    raise ValueError
            else:
                try:
            ...
            if not self.check_num_params(param):
                self.log.error(
                    "Incorrect number of parameters provided to neural network learner:"
                    + repr(param)
                    + ". Number of parameters:"
                    + str(self.num_params)
                )
            ...
            if not self.check_in_boundary(param):
                self.log.warning(
                    "Parameters provided to neural network learner not in boundaries:"
                    + repr(param)
                )
            cost = float(cost)
            uncer = float(uncer)
        ...
        if self.all_params.shape != (self.costs_count, self.num_params):
            self.log(
                "Saved NN params are the wrong size. THIS SHOULD NOT HAPPEN:"
                + repr(self.all_params)
            )
        if self.all_costs.shape != (self.costs_count,):
            self.log(
                "Saved NN costs are the wrong size. THIS SHOULD NOT HAPPEN:"
                + repr(self.all_costs)
            )
        if self.all_uncers.shape != (self.costs_count,):
            self.log(
                "Saved NN uncertainties are the wrong size. THIS SHOULD NOT HAPPEN:"
                + repr(self.all_uncers)
            )
    ...
    def find_next_parameters(self, net_index=None):
        ...
        for start_params in self.search_params:
            result = so.minimize(
                fun=lambda x: self.predict_cost(x, net_index),
                x0=start_params,
        ...
        try:
            while not self.end_event.is_set():
                ...
                for _ in range(self.generation_num):
                    ...
                    self.log.debug(
                        "Neural network learner generating parameter:"
                        + str(self.params_count + 1)
                    )
    ...
    def find_global_minima(self, net_index=None):
        ...
        self.predicted_best_cost = float(
            self.cost_scaler.inverse_transform([self.predicted_best_scaled_cost])
        )
        self.archive_dict.update(
            {
                "predicted_best_parameters": self.predicted_best_parameters,
